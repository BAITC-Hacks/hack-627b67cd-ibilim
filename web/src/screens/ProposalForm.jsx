...
import { createProposal, getPublishedTask } from '../api.js'
...
export default function ProposalForm({ id }) {
  ...
  useEffect(() => {
    ...
    setError('')
    setSubmitted(false)
    getPublishedTask(id).then((value) => { if (active) setTask(value) }).catch((cause) => { if (active) setError(cause.message) }).finally(() => { if (active) setLoading(false) })
    return () => { active = false }
  }, [id])
  ...
  return <section className="flow-page proposal-page"><div className="eyebrow">ЗАДАЧА №{task.id} · ДЛЯ КОМАНД</div><h1>{task.card.title || 'Задача бизнеса'}</h1><p className="flow-lead">{task.business?.name} · {task.industry}</p><div className="task-layout"><div className="task-main"><div className="surface task-readonly"><span className="section-kicker">О ЗАДАЧЕ</span><div className="readonly-fields">{meta?.fields?.filter(({ key }) => task.card?.[key]).map(({ key, label }) => <div key={key}><span>{label}</span><p>{key === 'contact' ? <ContactLinks value={task.card[key]} /> : task.card[key]}</p></div>)}</div></div>

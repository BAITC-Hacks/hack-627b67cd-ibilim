...
- `sources[поле]` — **дословная цитата** из черновика или ответа, на которой ИИ основал значение,
  ...
  отбрасывается и пишется в `ai.warnings`. Значение должно состоять из полных исходных фраз
  с сохранением отрицаний и условий; только контакт можно извлечь дословно из фразы.
  Свободное перефразирование не принимается — это защита от новых фактов (ТЗ §5).
...
### GET /api/catalog/{id}
Подробная карточка **для команды**, только из последней подтверждённой опубликованной версии.
Возвращает `id`, `status: "published"`, `business: {id, name}`, `industry`, `card` (все 10 полей),
`rating` (полный Rating по подтверждённой карточке, без `next_best[].then`),
`official: {score, level}`, `position: {place, of, projected: false}`, `proposals`, `published_at`.
Тексты и рейтинг остаются прежними после сохранения правок бизнеса и меняются только при
повторном подтверждении. Черновик, ответы на вопросы, источники, история и метаданные ИИ
в этот ответ не входят. `404` — задача не найдена или ещё не опубликована; низкий балл доступ
не ограничивает. Редактор бизнеса продолжает читать текущую версию через `GET /api/tasks/{id}`.
